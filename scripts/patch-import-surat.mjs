...
const handler = `  const parseImportedLetterText = (text: string, fileName: string) => {
    ...
    const bodyStart = Math.max(0, knownHeader + 1);
    const bodyLines = lines.slice(bodyStart).filter(l => !/^(?:Nomor|No\\.?|Lampiran|Perihal|Kepada Yth\\.?|Yth\\.?)\\b/i.test(l));
  ...
  const handleImportSuratFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (lower.endsWith('.docx')) {
        ...
        throw new Error('Format belum didukung. Gunakan .DOCX, .PDF, atau .TXT.');
      }
      if (!text.trim()) throw new Error('Teks surat tidak berhasil dibaca dari file. PDF hasil scan/foto membutuhkan OCR.');
      const imported = parseImportedLetterText(text, file.name);
      const nextNomor = imported.nomor_surat || generateNextNomorSurat(suratList);
      ...
      setFormData(prev => ({
        ...prev,
        ...imported,
        nomor_surat: nextNomor,
        logo_url: prev.logo_url || storedAssets.logo_url || DEFAULT_LOGO_URL,
        ttd_ketua_url: prev.ttd_ketua_url || storedAssets.ttd_ketua_url,
        ttd_sekretaris_url: prev.ttd_sekretaris_url || storedAssets.ttd_sekretaris_url,
        cap_stempel_url: prev.cap_stempel_url || storedAssets.cap_stempel_url
      }));
      ...
      Swal.fire({
        ...
        position: 'top-end',
        icon: 'success',
        title: 'Surat berhasil di-import',
        text: 'Isi dokumen sudah masuk ke formulir. Periksa lalu simpan ke arsip.',
        showConfirmButton: false,
        timer: 2800
      });
    } catch (err: any) {
      Swal.fire({
        title: 'Import Surat Gagal',
        text: err?.message || 'File tidak dapat diproses.',
        icon: 'error',
        confirmButtonColor: '#2563eb'
      });
    } finally {
      setIsImportingSurat(false);
    }
  };

`;
if (!s.includes(marker)) throw new Error('Import patch marker not found in KelolaSurat.tsx');
...
const button = `        <div className="relative z-10 flex items-center gap-2 shrink-0">
          {activeTab === 'keluar' && (
            <>
              <input
                ref={importSuratInputRef}
                type="file"
                accept=".docx,.pdf,.txt,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,text/plain"
                className="hidden"
                onChange={handleImportSuratFile}
              />
              <button
                type="button"
                onClick={() => importSuratInputRef.current?.click()}
                disabled={isImportingSurat}
                className="flex items-center gap-1.5 px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl transition-all text-[10px] sm:text-xs font-black uppercase tracking-widest active:scale-95 shrink-0 shadow-lg cursor-pointer text-white bg-emerald-600 hover:bg-emerald-500 shadow-emerald-600/20 disabled:opacity-60"
                title="Import surat dari Word/PDF"
              >
                {isImportingSurat ? <Loader2 size={14} className="animate-spin" /> : <Upload size={14} />}
                <span>{isImportingSurat ? 'Import...' : 'Import Surat'}</span>
              </button>
            </>
          )}
`;
if (!s.includes(buttonMarker)) throw new Error('Header button marker not found in KelolaSurat.tsx');
